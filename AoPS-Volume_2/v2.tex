...
\begin{a_problem}
	...
	Prove that $4 \mid n$.
	\begin{soln}
		We solve this problem first for the 2-number case: \\
		Every $-1$ is the result of a parity change from $x_i$ to $x_{i+1}$, so an even number of parity changes are required to get back where we started, and our total number of terms is twice that, so $4 \mid n$.
		
		This seems a bit hard to translate into the 4-case.
	\end{soln}
\end{a_problem}
